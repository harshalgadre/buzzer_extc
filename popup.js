...
function displaySessions(sessions) {
  ...
  if (sessions.length === 0) {
    ...
    noSessions.textContent =
      'No sessions created yet. Click "Create New" to get started.';
    noSessions.style.color = "white";
  ...
  sessions.forEach((session) => {
    ...
    sessionCard.innerHTML = `
      ...
      <p class="interview-position">${
        session.position || "Position not specified"
      } at ${session.company || "Company not specified"}</p>
      ...
      ${
        session.aiInterview
          ? '<span class="premium-badge">AI Interview</span>'
          : ""
      }
...
function setupEventListeners() {
  ...
  document
    .getElementById("website-login-btn")
    .addEventListener("click", showLoginForm);
  document
    .getElementById("form-back-btn")
    .addEventListener("click", hideLoginForm);
  document
    .getElementById("form-login-btn")
    .addEventListener("click", handleWebsiteLogin);
  document
...
async function handleSave() {
  ...
  const formData = {
    ...
    isDesktopCall: document
      .getElementById("desktop-toggle")
      .classList.contains("active"),
    liveCoding: document
      .getElementById("live-coding-toggle")
      .classList.contains("active"),
    aiInterview: document
      .getElementById("ai-interview-toggle")
      .classList.contains("active"),
...
function showScreen(screenName) {
  ...
  switch (screenName) {
    case "login":
      ...
      document.getElementById("google-login-btn").textContent =
...
async function startAIHelper(sessionId, meetingUrl) {
  try {
    console.log(
      "Starting AI Helper for session:",
      sessionId,
      "Meeting URL:",
      meetingUrl
    );
    ...
    console.log(
      "AI Helper request sent to background script with screen sharing"
    );
    ...
    if (error.name === "NotAllowedError") {
      alert(
        "Screen sharing permission denied. Please allow screen sharing to use AI Helper."
      );
    } else {
